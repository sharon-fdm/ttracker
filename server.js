          ...
          if (data) {
            const issues = JSON.parse(data)
              .filter(i => {
                ...
                return !statuses.some(s => s.includes('In review') || s.includes('Awaiting QA') || s.includes('Ready for release') || s.includes('Done') || s.includes('Confirm and celebrate'));
              })
              .map(i => {
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .sticky-color-btn:hover { border-color: #fdf6e3; }
  .modal-overlay {
    display: none;
    position: fixed;
...
function renderStickies() {
  ...
  board.innerHTML = stickies.map(s =>
    ...
    + '<div class="sticky-color-btn" style="background:#2aa198" onclick="changeStickyColor(\\'' + s.id + '\\', \\'#2aa198\\')"></div>'
    + '<div class="sticky-color-btn" style="background:#cb4b16" onclick="changeStickyColor(\\'' + s.id + '\\', \\'#cb4b16\\')"></div>'
...
function renderGantt(sprints) {
  ...
  container.innerHTML = '<div class="gantt">' + sprints.map((s, i) => {
    ...
    return '<div class="gantt-sprint' + (isCurrent ? ' current' : '') + '"' + (isPrevious ? ' style="opacity:0.6"' : '') + '>'
      ...
      + '<span style="color:var(--fg-muted)">' + startStr + ' - ' + endStr + '</span>'
      + '</div>'
