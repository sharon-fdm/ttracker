...
async function createPair(captainBadge, firstMateBadge, cwd) {
  ...
  // Open Captain terminal
  const captainBadgeB64 = Buffer.from(captainBadge).toString('base64');
  const captainScript = path.join(os.tmpdir(), `tt-pair-cap-${Date.now()}.applescript`);
  const captainCmd = safeMode
    ? `claude --name '${captainBadge}' --append-system-prompt "$(cat ${captainPromptFile})"`
    : `claude --dangerously-skip-permissions --name '${captainBadge}' --append-system-prompt "$(cat ${captainPromptFile})"`;

  fs.writeFileSync(captainScript, `tell application "iTerm2"
    set newWindow to (create window with default profile)
    tell current session of current tab of newWindow
        write text "cd '${cwdEscaped}'"
        delay 1
        write text "${captainCmd}"
    end tell
  ...
  let captainTty = '';
  try {
    captainTty = await server.runOsascriptFile(captainScript);
  } finally {
    try { fs.unlinkSync(captainScript); } catch {}
  }

  // Set Captain badge via TTY
  if (captainTty) {
    await new Promise(r => setTimeout(r, 1000));
    try { fs.writeFileSync(captainTty, `\x1b]1337;SetBadgeFormat=${captainBadgeB64}\x07`); } catch {}
  }
  ...
  const mateBadgeB64 = Buffer.from(firstMateBadge).toString('base64');
  const mateScript = path.join(os.tmpdir(), `tt-pair-mate-${Date.now()}.applescript`);
  const mateCmd = safeMode
    ? `claude --name '${firstMateBadge}' --append-system-prompt "$(cat ${matePromptFile})"`
    : `claude --dangerously-skip-permissions --name '${firstMateBadge}' --append-system-prompt "$(cat ${matePromptFile})"`;

  fs.writeFileSync(mateScript, `tell application "iTerm2"
    set newWindow to (create window with default profile)
    tell current session of current tab of newWindow
        write text "cd '${cwdEscaped}'"
        delay 1
  ...
  let mateTty = '';
  try {
    mateTty = await server.runOsascriptFile(mateScript);
  } finally {
    try { fs.unlinkSync(mateScript); } catch {}
  }

  // Set First Mate badge via TTY
  if (mateTty) {
    await new Promise(r => setTimeout(r, 1000));
    try { fs.writeFileSync(mateTty, `\x1b]1337;SetBadgeFormat=${mateBadgeB64}\x07`); } catch {}
  }
...
async function restorePair(historyIdx) {
  ...
  // Restore Captain
  const captainBadgeB64 = Buffer.from(entry.captain_badge).toString('base64');
  const captainScript = path.join(os.tmpdir(), `tt-pair-rcap-${Date.now()}.applescript`);
  const captainCmd = safeMode
    ? `claude --resume ${entry.captain_session_id} --append-system-prompt "$(cat ${captainPromptFile})"`
    : `claude --dangerously-skip-permissions --resume ${entry.captain_session_id} --append-system-prompt "$(cat ${captainPromptFile})"`;
  ...
  // Restore First Mate
  const mateBadgeB64 = Buffer.from(entry.first_mate_badge).toString('base64');
  const mateScript = path.join(os.tmpdir(), `tt-pair-rmate-${Date.now()}.applescript`);
  const mateCmd = safeMode
    ? `claude --resume ${entry.first_mate_session_id} --append-system-prompt "$(cat ${matePromptFile})"`
    : `claude --dangerously-skip-permissions --resume ${entry.first_mate_session_id} --append-system-prompt "$(cat ${matePromptFile})"`;
...
module.exports = { init, handlePairAPI };
