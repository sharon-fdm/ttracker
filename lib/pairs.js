...
async function autoRelay(pair, targetRole, commFile) {
  ...
  const msg = targetRole === 'first_mate'
    ? `Check the comm file for new messages from the Captain: cat "${commFile}" -- read the latest Captain message, fulfill the request, then append your response.`
    : `Your First Mate has responded. Read the comm file: cat "${commFile}" -- find the latest "First Mate:" line and report what they said.`;
